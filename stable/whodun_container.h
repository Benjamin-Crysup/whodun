...
/**Swap some vectors.*/
template <typename OfT>
void swap(StructVector<OfT>& vecA, StructVector<OfT>& vecB){
	uintptr_t tmpCap = vecA.curCap;
	uintptr_t tmpSize = vecA.curSize;
	OfT* tmpDats = vecA.datums;
	vecA.curCap = vecB.curCap;
	vecA.curSize = vecB.curSize;
	vecA.datums = vecB.datums;
	vecB.curCap = tmpCap;
	vecB.curSize = tmpSize;
	vecB.datums = tmpDats;
}
