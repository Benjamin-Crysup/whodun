...
extern HANDLE saveStdinHandle;
/**Save the standard output handle.*/
extern HANDLE saveStdoutHandle;
/**Save the standard error handle.*/
extern HANDLE saveStderrHandle;

};

using namespace whodun;

HANDLE whodun::saveStdinHandle = 0;
HANDLE whodun::saveStdoutHandle = 0;
HANDLE whodun::saveStderrHandle = 0;

ConsoleOutStream::ConsoleOutStream(){
	if(saveStdoutHandle == 0){
		HANDLE gotHand = GetStdHandle(STD_OUTPUT_HANDLE);
		if(gotHand == INVALID_HANDLE_VALUE){
			throw std::runtime_error("Could not get stdout.");
		}
		if(gotHand){
			saveStdoutHandle = gotHand;
		}
	}
}
ConsoleOutStream::~ConsoleOutStream(){}
...
void ConsoleOutStream::write(const char* toW, uintptr_t numW){
	if(saveStdoutHandle){
		...
		while(numLeft){
			...
			DWORD numWritten;
			if(WriteFile(saveStdoutHandle, curW, curNumR, &numWritten, 0)==0){
				throw std::runtime_error("Problem writing to stdout.");
			}
			curW += curNumR;
			numLeft -= curNumR;
		}
...
ConsoleErrStream::ConsoleErrStream(){
	if(saveStderrHandle == 0){
		HANDLE gotHand = GetStdHandle(STD_ERROR_HANDLE);
		if(gotHand == INVALID_HANDLE_VALUE){
			throw std::runtime_error("Could not get stderr.");
		}
		if(gotHand){
			saveStderrHandle = gotHand;
		}
	}
}
ConsoleErrStream::~ConsoleErrStream(){}
...
void ConsoleErrStream::write(const char* toW, uintptr_t numW){
	if(saveStderrHandle){
		...
		while(numLeft){
			DWORD curNumR = (numLeft > 0x10000000) ? 0x10000000 : numLeft;
			DWORD numWritten;
			if(WriteFile(saveStderrHandle, curW, curNumR, &numWritten, 0)==0){
				throw std::runtime_error("Problem writing to stderr.");
			}
			curW += curNumR;
			numLeft -= curNumR;
		}
...
ConsoleInStream::ConsoleInStream(){
	if(saveStdinHandle == 0){
		HANDLE gotHand = GetStdHandle(STD_INPUT_HANDLE);
		if(gotHand == INVALID_HANDLE_VALUE){
			throw std::runtime_error("Could not get stdin.");
		}
		if(gotHand){
			saveStdinHandle = gotHand;
		}
	}
}
ConsoleInStream::~ConsoleInStream(){}
...
uintptr_t ConsoleInStream::read(char* toR, uintptr_t numR){
	if(saveStdinHandle){
		...
		while(numLeft){
			DWORD curNumR = (numLeft > 0x10000000) ? 0x10000000 : numLeft;
			DWORD numGot;
			if(ReadFile(saveStdinHandle, curR, curNumR, &numGot, 0) == 0){
